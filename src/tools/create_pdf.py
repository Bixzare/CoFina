...
PLANS_DIR = os.path.join(_HERE, "..", "pdf_files")
...
def _render_goals(story, goals_raw, normal_style):
    ...
    if isinstance(goals_raw, str):
        try:
            ...
            story.append(Paragraph(f"• {goals_raw}", normal_style))
            return
...
def create_financial_plan_pdf(
    ...
    try:
        ...
        doc = SimpleDocTemplate(filepath, pagesize=letter,
                                ...
                                title=f"CoFina – {plan_name} – {user_id}",
                                author="CoFina AI")
        S = _make_styles()
        story = []
        ...
        if debts:
            ...
            story.append(Paragraph(f"💡 Your strategy: {tip}", S["small"]))
            story.append(Spacer(1, 0.12 * inch))

        # Goals
        story.append(Paragraph("Financial Goals", S["heading"]))
        story.append(Paragraph("Short-term (1–2 years)", S["subheading"]))
        ...
        for a in [
            "1. <b>Build Emergency Fund</b> — 3–6 months of expenses before investing.",
            "2. <b>Automate savings</b> — set up automatic transfers on payday.",
            "3. <b>Attack debt</b> — follow your chosen repayment strategy consistently.",
            "4. <b>Review monthly</b> — track spending and adjust budget categories.",
            "5. <b>Maximise retirement contributions</b> — capture any employer match first.",
            "6. <b>Invest the surplus</b> — low-cost index funds for long-term wealth.",
            "7. <b>Annual review</b> — update this plan every year or after major life events.",
        ]:
...
def create_topic_plan_pdf(
    ...
    try:
        ...
        doc = SimpleDocTemplate(filepath, pagesize=letter,
                                topMargin=0.75*inch, bottomMargin=0.75*inch,
                                leftMargin=0.75*inch, rightMargin=0.75*inch,
                                title=f"CoFina – {plan_label} – {user_id}",
                                author="CoFina AI")
        S = _make_styles()
        story = []
        ...
        for line in plan_content.splitlines():
            ...
            if line.startswith("**") and line.endswith("**"):
                ...
                story.append(Paragraph("• " + line.lstrip("*-• "), S["normal"]))
            else:
                story.append(Paragraph(line, S["normal"]))
        story.append(Spacer(1, 0.12 * inch))
...
def create_pdf(
    ...
    """
    Create a PDF document for any content the user requests.

    This tool is flexible and handles all of the following:
    - Financial plans (full profile-based plans with projections and budgets)
    - Topic-specific financial plans (car purchase, house, debt payoff, savings, etc.)
    - Financial advice (tips, strategies, recommendations)
    - Definitions and explanations of financial terms or concepts
    - Any other content the user wants saved as a PDF

    Parameters
    ----------
    content        : The main text or body of the document. For financial plans this
                     is the plan text; for definitions/advice it is the explanation.
    title          : Title/name for the document (e.g. "Car Purchase Plan",
                     "What is Compound Interest", "Investment Advice").
    user_id        : The ID of the requesting user (default: "guest").
    content_type   : Hint for layout — one of:
                       "financial_plan"  → full profile plan with projections,
                       "topic_plan"      → topic-specific plan,
                       "advice"          → financial advice,
                       "definition"      → term/concept explanation,
                       "general"         → any other content (default).
    profile_data   : Optional JSON string of the user profile (used for
                     personalised financial plans).
    short_term_goals : Optional short-term goals string (used when
                     content_type="financial_plan").
    long_term_goals  : Optional long-term goals string (used when
                     content_type="financial_plan").
    include_projections : Whether to include savings projections table
                     (only relevant for full financial plans).
    """
    if content_type == "financial_plan" and profile_data and short_term_goals and long_term_goals:
        # Full profile-based financial plan
        ...
        # Topic plan, advice, definition, or any general content
        profile = {}
        if profile_data:
            try:
                ...
                profile = {}
        return create_topic_plan_pdf(
            user_id=user_id,
            topic=content_type if content_type not in ("general", "definition", "advice") else title.lower().replace(" ", "_"),
